...
use crate::{
  index::reorg::Reorg,
  runebeta::{
    models::{NewOutPointValue, NewOutpointSatRange},
    storage::{
      height_sequence_number::HeightSequenceNumberTable, home_inscription::HomeInscriptionTable,
      BlockHeaderTable, InscriptionEntryTable, OutPointSatrangeTable, OutPointValueTable,
      RuneEntryTable, SatPointTable,
    },
    SatRange, Statistic,
  },
  schema::indexing_block_timestamps,
  settings::Settings,
...
use super::{
  ...
  models::NewIndexingBlockTimestamp,
  storage::{self, statistic::StatisticTable},
...
pub(crate) struct Updater<'index> {
  ...
  pub(super) range_cache: HashMap<OutPoint, Vec<u8>>,
  pub(super) sat_ranges_since_flush: u64,
}

impl<'index> Updater<'index> {
  pub(crate) fn update_index<'a>(&'a mut self, wtx: &mut WriteTransaction) -> Result {
    ...
    if starting_index_height == 0 && self.height > 0 {
      let statistic_table = StatisticTable::new(wtx.get_connection());
      statistic_table.set_initial_sync_time(u64::try_from(start.elapsed().as_micros())?)?;
  ...
  fn index_block(
    ...
    let mut outpoint_value_table = OutPointValueTable::new(wtx.get_connection());
    ...
    if index_inscriptions {
      ...
      for (tx, _) in &block.txdata {
        for input in &tx.input {
          ...
          if outpoint_value_table.get(&prev_output.store())?.is_some() {
            continue;
          }
    ...
    let statistic_table = StatisticTable::new(wtx.get_connection());
    let block_header_table = BlockHeaderTable::new(wtx.get_connection());
    let mut statistic_to_count = statistic_table.get_or_create_indexing_statistic()?;
    let mut height_sequence_number_table = HeightSequenceNumberTable::new(wtx.get_connection());
    let mut inscription_entry_table = InscriptionEntryTable::new(wtx.get_connection());
    let mut home_inscription_table = HomeInscriptionTable::new(wtx.get_connection());
    ...
    let mut lost_sats = statistic_to_count.map(|stat| stat.lost_sats).unwrap_or(0);
    ...
    //   .unwrap_or(0);
    let next_sequence_number = inscription_entry_table
    ...
    let home_inscription_count = home_inscription_table.count()?;
    let mut inscription_updater = InscriptionUpdater::new(
      statistic_to_count.blessed_inscriptions,
      self.index.settings.chain(),
      statistic_to_count.cursed_inscriptions,
      ...
      block.header.time,
      statistic_to_count.unbound_inscriptions,
      value_cache,
      value_receiver,
      wtx,
    );
    ...
    if self.index.index_sats {
      let mut satpoint_table = SatPointTable::new(wtx.get_connection());
      // let mut sat_to_satpoint = wtx.open_table(SAT_TO_SATPOINT)?;
      // Outpoint_to_satrange
      let mut outpoint_sat_range_table = OutPointSatrangeTable::new(wtx.get_connection());
        ...
        for input in &tx.input {
          ...
          let sat_ranges = match if self.index.index_spent_sats {
            ...
            None => if self.index.index_spent_sats {
              outpoint_sat_range_table.get(&key)?
            } else {
              outpoint_sat_range_table.remove(&key)?
            }
      ...
      if !coinbase_inputs.is_empty() {
        let mut lost_sat_ranges = outpoint_sat_range_table
        ...
        let outpoint_sat_range = NewOutpointSatRange {
          tx_hash: todo!(),
          vout: todo!(),
          range: todo!(),
        };
        outpoint_sat_range_table.insert(&OutPoint::null().store(), lost_sat_ranges.as_slice())?;
      }
    ...
    if index_inscriptions {
      height_sequence_number_table
        .insert(&self.height, inscription_updater.next_sequence_number)?;
    }

    statistic_to_count.lost_sats = if self.index.index_sats {
      lost_sats
    } else {
      inscription_updater.lost_sats
    };

    statistic_to_count.cursed_inscriptions = inscription_updater.cursed_inscription_count;
    statistic_to_count.blessed_inscriptions = inscription_updater.blessed_inscription_count;
    inscription_updater.unbound_inscriptions = inscription_updater.unbound_inscriptions;
    //Save statistic
    statistic_table.update_indexing_statistic(&statistic_to_count)?;
    if self.index.index_runes && self.height >= self.index.settings.first_rune_height() {
      ...
      let runes = statistic_to_count.runes;
      ...
      let rune_entry_table = RuneEntryTable::new(wtx.get_connection());
      ...
      for (rune_id, update) in rune_updater.updates {
        // let mut Rune = rune_entry_table.get(&rune_id);
        // let mut entry = RuneEntry::load(rune_table.get(&rune_id.store())?.unwrap().value());
        let mut entry = rune_entry_table.get(&rune_id)?;
        entry.burned += update.burned;
        entry.mints += update.mints;
        entry.supply += update.supply;

        rune_entry_table.update(&entry)?;
      }
    }
    block_header_table.set_block_header(&self.height, &block.header)?;
  ...
  fn index_transaction_sats(
    ...
    tx: &Transaction,
    txid: Txid,
    satpoint_table: &mut storage::SatPointTable,
  ...
  fn commit(&mut self, wtx: WriteTransaction, value_cache: HashMap<OutPoint, u64>) -> Result {
    ...
    if self.index.index_sats {
      ...
      let mut outpoint_sat_ranges = OutPointSatrangeTable::new(wtx.get_connection());
      for (outpoint, sat_ranges) in self.range_cache.drain() {
        ...
        //outpoint_sat_ranges.create(&outpoint, sat_ranges.as_slice())?;
        outpoint_sat_ranges.upsert_values(&outpoint_sat_range)?
      }
    ...
    {
      //let mut outpoint_to_value = wtx.open_table(OUTPOINT_TO_VALUE)?;
      let outpoint_value_table = OutPointValueTable::new(wtx.get_connection());
      let mut outpoint_values = Vec::new();
      for (outpoint, value) in value_cache {
        outpoint_values.push(NewOutPointValue {
          tx_hash: outpoint.txid.to_string().as_str(),
          vout: outpoint.vout,
          value,
        });
      ...
      outpoint_value_table.insert(&outpoint_values);
    }
