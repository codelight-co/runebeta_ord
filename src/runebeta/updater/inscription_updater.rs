...
use crate::{
  ...
  runebeta::{
    ...
    models::{
      NewContentTypeCount, NewInscriptionEntry, NewSatpointEntity, NewTransaction,
      NewTransactionIn, NewTransactionOut,
    },
  },
...
use self::{
  ...
  event::Event,
  storage::{ContentTypeCountTable, TransactionTable},
};
...
pub(super) struct InscriptionUpdater<'a, 'db> {
  ...
  pub(super) transactions: TransactionTable<'db>,
  ...
  pub(super) inscription_entry: InscriptionEntryTable<'db>,
  //pub(super) sequence_number_to_satpoint: &'a mut Table<'db, 'tx, u32, &'static SatPointValue>,
  pub(super) satpoints: SatPointTable<'db>,
...
impl<'a, 'db> InscriptionUpdater<'a, 'db> {
  pub fn new(
    ...
    Self {
      ...
      chain,
      content_type_to_count: wtx.create_content_type_count_table(),
      cursed_inscription_count,
      event_sender,
      flotsam,
      height,
      home_inscription_count,
      home_inscriptions: wtx.create_home_inscription_table(),
      ...
      reward,
      transactions: wtx.create_transaction_table(),
      //sat_to_sequence_number: todo!(),
      //satpoint_to_sequence_number: todo!(),
      //sequence_number_to_children: todo!(),
      inscription_entry: wtx.create_inscription_entry_table(),
      //sequence_number_to_satpoint: todo!(),
      satpoints: wtx.create_satpoint_table(),
      timestamp,
  ...
  fn update_inscription_location(
    ...
    self.satpoints.insert(&sequence_number, &satpoint)?;
    // self
    //   .satpoint_to_sequence_number
