...
impl RuneBetaIndex {
  ...
  pub fn open_with_event_sender(
    ...
    //   }
    // };
    let mut pg_connection = establish_pgconnection();
    /*
    ...
    */
    ...
    let index_spent_sats = statistics.index_spent_sats;
    let index_transactions = statistics.index_spent_sats;
  ...
  pub fn update(&self) -> Result {
    loop {
      ...
      let last_height = block_header_table.get_indexed_block();
      // let last_height = wtx
      //   .open_table(HEIGHT_TO_BLOCK_HEADER)?
      //   .range(0..)?
      //   .next_back()
  ...
  //   let rtx = self.database.begin_read()?;
  //   let block_header_table = BlockHeaderTable::new(wtx.get_connection());
  //   let blocks_indexed = block_header_table.get_indexed_block()?;
  ...
  // }
  /*
   * Mar 19, TaiVV, using for indexing server
   */
...
mod tests {
  use {
    super::*,
    crate::runebeta::{storage::statistic::StatisticTable, testing::Context},
  };
  ...
  //     let wtx = context.index.database.begin_write().unwrap();
  //     let &mut connection = context.index.get_Con
  //     wtx
  //       .open_table(STATISTIC_TO_COUNT)
  //       .unwrap()
